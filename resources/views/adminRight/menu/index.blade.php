...
@section('content')
    ...
    @include('public.left')
    <a href="{{ url('adminRight/menu/create') }}" class="layui-btn">添加菜单</a>
    ...
    <script>
            ...
            var re = treeTable.render({
                ...
                cols: [
                    ...
                    },
                    {
                        ...
                        width: '100px',
                        template: function (item) {
                            var html = '<a href="{{ url('adminRight.menu.edit') }}?menu_id=' + item.id + '" class="layui-btn layui-btn-xs">编辑</a>';
        ...
        function del(id) {
            layer.confirm('你确定要删除这个菜单吗？', {
                ...
                var load = layer.load();
                $.post("{{ url('adminRight/menu/delete') }}", {id: id},
                    function (data) {
                        layer.close(load);
                        if (data.code === 0) {
                            layer.msg('操作成功', {
                                ...
                                location.href = "{{ url('adminRight/menu') }}";
                            });
                        } else {
                            layer.msg(data.msg, {
                                offset: '15px'
