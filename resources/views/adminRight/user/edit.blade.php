...
@section('content')
    ...
    @include('public.left')
    <a href="{{ url('adminRight/user') }}" class="layui-btn layui-btn-primary layui-btn-sm">返回</a>
    <hr>
        ...
        {{--<div class="layui-form-item">--}}
            ...
            {{--<div class="layui-input-block">--}}
                {{--<input type="radio" name="status" value="1" title="启用"--}}
                       {{--@if($user && $user->status == \App\Model\Users::STATUS_ENABLE)--}}
                       {{--checked--}}
                        {{--@endif--}}
                {{-->--}}
                {{--<input type="radio" name="status" value="2" title="禁用"--}}
                       {{--@if($user && $user->status == \App\Model\Users::STATUS_DISABLE)--}}
                       {{--checked--}}
                        {{--@endif--}}
                {{-->--}}
            {{--</div>--}}
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="radio" name="administrator" value="1" lay-filter="admin" title="是"
                       @if($user && $user->administrator == \App\Model\Users::ADMIN_YES)
                       checked
                        @endif
                >
                <input type="radio" name="administrator" value="2" lay-filter="admin" title="否"
                       @if($user && $user->administrator == \App\Model\Users::ADMIN_NO)
                       checked
                        @endif
                >
            </div>
    ...
    <script>
        layui.use(['form', 'layer'], function () {
            ...
            layer.msg('{{ $error }}', {
                ...
                location.href = "{{ url('adminRight/user') }}";
            });
            @endif
            ...
            form.on('submit(formDemo)', function (data) {
                ...
                var load = layer.load();
                $.post("{{ url('adminRight/user/update') }}", data.field,
                    function (data) {
                        layer.close(load);
                        if (data.code === 0) {
                            layer.msg('操作成功', {
